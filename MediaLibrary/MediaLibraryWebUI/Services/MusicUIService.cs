...
namespace MediaLibraryWebUI.Services
{
    ...
    public class MusicUIService : BaseUIService, IMusicUIService
    {
        ...
        public async Task<IEnumerable<IGrouping<string, Album>>> GetAlbumGroups(AlbumSort sort)
        {
            ...
            if (albums == null) /*then*/ albums = (await dataService.GetList<Album>(default, default, album => album.Artist, album => album.Track))
                                                         .Where(album => album.Track.Any());
        ...
        public async Task<IEnumerable<IGrouping<string, Artist>>> GetArtistGroups(ArtistSort sort)
        {
            ...
            if (artists == null) /*then*/ artists = (await dataService.GetList<Artist>(default, default, artist => artist.Album))
                                                           .Where(artist => artist.Album.Any());
